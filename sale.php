...
if(!empty($_GET['dnid'])){
$sql = "DELETE FROM sale WHERE (id='".$_GET['dnid']."')";  
mysql_query($sql) or die(mysql_error());

                        header("Location: sale.php?");
}

if(!empty($_POST['uid']) and empty($_POST['esid'])){

$date = mktime(date("H"),date("i"),date("s"),date("m"),date("d"),date("Y"));
$sql = "INSERT INTO sale VALUES ('', '".addslashes($_POST['uid'])."',
...
if(!empty($_POST['esid'])){
$sql = "UPDATE sale SET uid='".addslashes($_POST['uid'])."',
cid='".addslashes($_POST['cid'])."',
eid='".addslashes($_POST['eid'])."' WHERE (id='".$_POST['esid']."')";
mysql_query($sql) or die(mysql_error());
...
$sql = "SELECT (SELECT name FROM users WHERE(sale.uid=users.id)),
(SELECT fio FROM employee WHERE(sale.eid=employee.id)),
(SELECT name FROM credit WHERE(sale.cid=credit.id)),sale.* FROM sale ";
...
$sql = "SELECT * FROM sale WHERE(id='".$_GET['eid']."')";
     $s=mysql_query("$sql") or die(mysql_error());
     $rows = mysql_num_rows($s);
            $f=mysql_fetch_array($s);
...
<label class="control-label">Пользователь <span class="required">
* </span>
</label>
