...
$titleP = 'Отчет по выполнению плана';  
?><!DOCTYPE html>
...
<?php $pageid=10; include('menu.php'); ?>
...
$sql = "SELECT (SELECT `text` FROM sale_plan WHERE(users.id=sale_plan.uid)),
(SELECT COUNT(sum3) FROM stat WHERE(date>'".$date1."' and date<'".$date2."') and status=0),
users.* FROM users ";
