...
(SELECT fio FROM employee WHERE(employee.id=stat.eid)),(SELECT name FROM credit WHERE(credit.id=stat.credit)),stat.* FROM stat WHERE(date>'".$date1."' and date<'".$date2."') and status=1 ORDER BY stat.id DESC";
...
$sql = "SELECT (SELECT name FROM users WHERE(users.id=stat.uid)),
(SELECT fio FROM employee WHERE(employee.id=stat.eid)),(SELECT name FROM credit WHERE(credit.id=stat.credit)),stat.* FROM stat WHERE(date>'".$date1."' and date<'".$date2."') and status=0 ORDER BY stat.id DESC";
     $s=mysql_query($sql) or die(mysql_error());
     $rows = mysql_num_rows($s);
echo '
<h2>Всего расчетов '.$rows.'</h2>
  <table class="table table-bordered">
<thead>
<tr>
<th>Дата</th>
